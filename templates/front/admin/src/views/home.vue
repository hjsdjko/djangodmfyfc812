...
<div class="content" :style='{"minHeight":"100vh","padding":"20px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231022/30183857b9844112ade95e282459e4ba.png) no-repeat center top / cover","display":"flex","fontSize":"14px","height":"auto"}'>
	...
	<div :style='{"border":"0px solid #bababa","padding":"0px","margin":"0 0 30px","alignItems":"flex-start","color":"#ababab","display":"block","justifyContent":"flex-start","minHeight":"394px","borderRadius":"0px","flexWrap":"wrap","background":"noner","flex":"1","width":"49%","fontSize":"14px","order":"0"}'>
		<div :style='{"padding":"20px 10px","margin":"0 2% 12px 0","borderColor":"#ddd","alignItems":"center","textAlign":"center","display":"flex","float":"left","justifyContent":"flex-start","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(240,243,244,.5) 100%),#fff","borderWidth":"1px","width":"48%","borderStyle":"solid"}' v-if="isAuth('cheweixinxi','首页总数')">
			...
			<div :style='{"width":"auto","margin":"0 0 0 20px","textAlign":"left","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"500","height":"24px"}'>{{cheweixinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#737373","height":"24px"}'>车位信息总数</div>
			</div>
		</div>
		<div :style='{"padding":"20px 10px","margin":"0 2% 12px 0","borderColor":"#ddd","alignItems":"center","textAlign":"center","display":"flex","float":"left","justifyContent":"flex-start","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(240,243,244,.5) 100%),#fff","borderWidth":"1px","width":"48%","borderStyle":"solid"}' v-if="isAuth('cheliangruchang','首页总数')">
			...
			<div :style='{"width":"auto","margin":"0 0 0 20px","textAlign":"left","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"500","height":"24px"}'>{{cheliangruchangCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#737373","height":"24px"}'>车辆入场总数</div>
			</div>
		</div>
		<div :style='{"padding":"20px 10px","margin":"0 2% 12px 0","borderColor":"#ddd","alignItems":"center","textAlign":"center","display":"flex","float":"left","justifyContent":"flex-start","borderRadius":"0px","background":"linear-gradient(180deg, rgba(255,255,255,1) 0%, rgba(240,243,244,.5) 100%),#fff","borderWidth":"1px","width":"48%","borderStyle":"solid"}' v-if="isAuth('cheliangchuchang','首页总数')">
			...
			<div :style='{"width":"auto","margin":"0 0 0 20px","textAlign":"left","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"500","height":"24px"}'>{{cheliangchuchangCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#737373","height":"24px"}'>车辆出场总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type3" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"10"}'>
		<div id="cheweixinxiChart1" class="echarts1" v-if="isAuth('cheweixinxi','首页统计')"></div>
		<div id="cheliangruchangChart1" class="echarts2" v-if="isAuth('cheliangruchang','首页统计')"></div>
		<div id="cheliangchuchangChart1" class="echarts3" v-if="isAuth('cheliangchuchang','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		...
		this.getcheliangruchangCount();
		this.cheliangruchangChat1();
	...
	methods:{
        ...
        this.$http({
            url: "cheweixinxi/group/cheweizhuangtai",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
		...
		getcheliangruchangCount() {
			this.$http({
				url: `cheliangruchang/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.cheliangruchangCount = data.data
				}
			})
		},
		cheliangruchangChat1() {
			this.$nextTick(()=>{

        var cheliangruchangChart1 = echarts.init(document.getElementById("cheliangruchangChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                cheliangruchangChart1.setOption(option);
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, cheliangruchangChart1)
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    cheliangruchangChart1.resize();
                };
            }
        });
      })
        ...
        this.$http({
            url: `cheliangchuchang/value/chuchangshijian/tingchefeiyong/日`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].chuchangshijian);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].chuchangshijian
                    })
                }
                var option = {};
