<template>
	<div class="main-content" :style='{"color":"#918C8C","padding":"20px 30px 30px","fontSize":"14px"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"0px","margin":"0px","overflow":"hidden","borderRadius":"0px","background":"none","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px","boxShadow":"0 1px 0px rgba(0,0,0,0.12), 0 1px 1px rgba(0,0,0,0.24)","textAlign":"center","flexWrap":"wrap","background":"#fff","flexDirection":"row-reverse","display":"flex","width":"100%","fontSize":"inherit","justifyContent":"center"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">入场编号</label>
						<el-input v-model="searchForm.ruchangbianhao" placeholder="入场编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">车位编号</label>
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">车主号</label>
						<el-input v-model="searchForm.chezhuhao" placeholder="车主号" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"20px","boxShadow":"0 1px 0px rgba(0,0,0,0.12), 0 1px 1px rgba(0,0,0,0.24)","margin":"20px 0 10px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row-reverse","background":"#fff","display":"flex","width":"100%","fontSize":"inherit","justifyContent":"center"}'>
					...
					<el-button class="btn18" v-if="isAuth('cheliangruchang','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
						<span class="icon iconfont icon-shenhe2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						审核
					</el-button>

					<el-button class="btn18" v-if="isAuth('cheliangruchang','车辆入场统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="cheweitupian" width="200" label="车位图片">
						<template slot-scope="scope">
							<div v-if="scope.row.cheweitupian">
								<img v-if="scope.row.cheweitupian.substring(0,4)=='http'" :src="scope.row.cheweitupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
								<img v-else :src="$base.url+scope.row.cheweitupian.split(',')[0]" width="100" height="100" style="object-fit: cover">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('cheliangruchang','出场')" @click="cheliangchuchangCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','请勿重复操作')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								出场
							</el-button>
							<el-button class="edit" v-if=" isAuth('cheliangruchang','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="chepaihaoChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var chepaihaoChart1 = echarts.init(document.getElementById("chepaihaoChart1"),'macarons');
        this.$http({
            url: "cheliangruchang/group/chepaihao",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                chepaihaoChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chepaihaoChart1.resize();
                };
            }
        });
      })
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.ruchangbianhao!='' && this.searchForm.ruchangbianhao!=undefined){
            params['ruchangbianhao'] = '%' + this.searchForm.ruchangbianhao + '%'
          }
           ...
           if(this.searchForm.chezhuhao!='' && this.searchForm.chezhuhao!=undefined){
            params['chezhuhao'] = '%' + this.searchForm.chezhuhao + '%'
          }
		...
		this.$refs["shBatchForm"].validate(valid => {
			if(valid){
				this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
					...
					this.$http({
						url: "cheliangruchang/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
						method: "post",
						data: this.batchIds
					}).then(({ data }) => {
						if (data && data.code === 0) {
