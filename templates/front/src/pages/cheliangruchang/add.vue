...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="120px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="入场编号" prop="ruchangbianhao">
              <el-input v-model="ruleForm.ruchangbianhao" placeholder="入场编号" disabled></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="车位编号" prop="cheweibianhao">
            <el-input v-model="ruleForm.cheweibianhao" 
                placeholder="车位编号" clearable :disabled=" false  ||ro.cheweibianhao"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="车位位置" prop="cheweiweizhi">
            <el-input v-model="ruleForm.cheweiweizhi" 
                placeholder="车位位置" clearable :disabled=" false  ||ro.cheweiweizhi"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="车位图片" v-if="type!='cross' || (type=='cross' && !ro.cheweitupian)" prop="cheweitupian">
            <file-upload
            tip="点击上传车位图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' class="upload" v-else label="车位图片" prop="cheweitupian">
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="每小时价" prop="meixiaoshijia">
			<el-input-number v-model="ruleForm.meixiaoshijia" placeholder="每小时价" :disabled="ro.meixiaoshijia"></el-input-number>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="车主号" prop="chezhuhao">
            <el-input v-model="ruleForm.chezhuhao" 
                placeholder="车主号" clearable :disabled=" false  ||ro.chezhuhao"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="车主姓名" prop="chezhuxingming">
            <el-input v-model="ruleForm.chezhuxingming" 
                placeholder="车主姓名" clearable :disabled=" false  ||ro.chezhuxingming"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="入场时间" prop="ruchangshijian">
              <el-date-picker
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.chezhuhao!=''&&json.chezhuhao) || json.chezhuhao==0){
                this.ruleForm.chezhuhao = json.chezhuhao;
				this.ro.chezhuhao = true;
            }
            if((json.chezhuxingming!=''&&json.chezhuxingming) || json.chezhuxingming==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`cheliangruchang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('cheliangruchang/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`cheliangruchang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
